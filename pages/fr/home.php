...
<section class="carousel">
    <div class="container">
        <img src="images/carouselhome/Openng sales discounts.png" alt="Promotions d'ouverture - Produits à base d'huile d'argan">
        <img src="images/carouselhome/Openng sales discounts (1).png" alt="Offres spéciales sur l'huile d'argan - CoopArjana">
    </div>
    <div class="buttons">
        ...
        <div class="cat">
            <?php foreach ($categories as $cat): ?>
                <div onclick="goToCategory(<?= $cat['id'] ?>)">
                    <img src="<?= $cat['main_image'] ?? 'images/default.jpg' ?>" alt="<?= $cat['name_fr'] ?>">
                    <h3><?= $cat['name_fr'] ?></h3>
                </div>
            <?php endforeach; ?>
        </div>
                    ...
                    foreach ($popular_products as $row) {
                        ...
                        <a href="index.php?page=product&id=<?= $product_id ?>" class="product-card-link">
                            <div class="product-card" data-id="<?= $product_id ?>">
                                ...
                                <?php if ($has_variants): ?>
                                    <a href="index.php?page=product&id=<?= $product_id ?>"><button>Plus de détails</button></a>
                                <?php else: ?>
                    ...
                    foreach ($last_products as $row) {
                        ...
                        <a href="index.php?page=product&id=<?= $product_id ?>" class="product-card-link">
                            <div class="product-card" data-id="<?= $product_id ?>">
                                ...
                                <p><?= $price_display . ' ' . $currencyLabel ?></p>
                                <?php if ($has_variants): ?>
                                    <button onclick="event.stopPropagation(); window.location.href='index.php?page=product&id=<?= $product_id ?>'">Plus de détails</button>
                                <?php else: ?>
                                    <button onclick="event.preventDefault(); ajouterDepuisProduits(this)">+ Ajouter</button>
                                <?php endif; ?>
                            </div>
                        </a>
                        <?php
