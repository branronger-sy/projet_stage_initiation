...
if ($q === '') {
    ...
    echo "Valeur de q : [" . htmlspecialchars($q) . "]";
    exit;
}
...
try {
    $stmt = $pdo->prepare("
        ...
        WHERE LOWER(p.name_fr) LIKE LOWER(:q1)
           OR LOWER(p.description_fr) LIKE LOWER(:q2)
           OR LOWER(v.var_name_fr) LIKE LOWER(:q3)
    ...
    foreach ($products as $p) {
        // Si la variante a un nom différent, on l’affiche
        $name = !empty($p['variant_name']) ? $p['variant_name'] : $p['product_name'];

        // Clé unique pour éviter la répétition
