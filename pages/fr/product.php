...
if (!isset($pdo) || !($pdo instanceof PDO)) {
    error_log('product.php: $pdo non disponible');
    echo "<h2>Erreur interne (connexion DB).</h2>";
    exit;
}
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    echo "<h2>ID produit invalide.</h2>";
    exit;
}
...
if ($convertedProductPrice === null) {
    error_log("product.php: produit id {$product_id} n'a pas de prix numérique en DB. Clés du produit: " . implode(',', array_keys($product)));
}
$cat_stmt = $pdo->prepare("SELECT name_fr FROM categories WHERE id = ?");
$cat_stmt->execute([$product['category_id']]);
$category = $cat_stmt->fetchColumn();
$breadcrumb = "Accueil > " . htmlspecialchars($category) . " > " . htmlspecialchars($product['name_fr']);
?>

<main class="product-container" data-selected-variant="">
  <section class="gallery">
    <div class="main-image">
      <img id="main-img" src="<?= htmlspecialchars(fullPath($product['image_url']), ENT_QUOTES, 'UTF-8') ?>" alt="Image principale du produit">
    </div>
    <div class="thumbnail-container">
      <?php if (!empty($product['image_url'])): ?>
        <img src="<?= htmlspecialchars(fullPath($product['image_url']), ENT_QUOTES, 'UTF-8') ?>" onclick="changeImage(this)" alt="Miniature" class="active">
      <?php endif; ?>
      <?php foreach ($other_images as $img): ?>
        <img src="<?= htmlspecialchars(fullPath($img['image_url']), ENT_QUOTES, 'UTF-8') ?>" onclick="changeImage(this)" alt="Miniature">
      <?php endforeach; ?>
    </div>
  </section>
  ...
  <section class="product-info">
    ...
    <h1><?= htmlspecialchars($product['name_fr']) ?></h1>
    <div class="price">
...
<div class="description">
  <h2>Description</h2>
  <p><?= htmlspecialchars($product['description_fr'] ?? '') ?></p>
</div>
...
<?php else: ?>
  <div class="out-of-stock-msg" style="font-size: 1.3em; color: #178768">Le produit est actuellement indisponible et sera bientôt disponible.</div>
<?php endif; ?>


<div class="product-meta">
  <p><strong>Disponibilité :</strong> 
    <?= ((int)$product['stock'] > 0) ? 'En stock' : 'Rupture de stock' ?>
  </p>
