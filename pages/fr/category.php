...
if ($category_id > 0) {
    $stmt = $pdo->prepare("SELECT id, name_fr FROM categories WHERE id = :cid LIMIT 1");
    ...
    if ($category) {
        $stmt = $pdo->prepare("
            SELECT 
                p.id AS product_id,
                COALESCE(v.var_name_fr, p.name_fr) AS name,
...
<main>
  <?php if (!empty($category)): ?>
    <section>
      <h1 id="category-title">
        <?= htmlspecialchars($category['name_fr'] ?? '', ENT_QUOTES, 'UTF-8'); ?>
      </h1>
      <p id="category-description">
        Découvrez nos meilleurs produits dans la catégorie 
        <?= htmlspecialchars($category['name_fr'] ?? '', ENT_QUOTES, 'UTF-8'); ?>.
      </p>
    </section>
    ...
    <section id="products" class="products-container">
      <?php if (!empty($products)): ?>
        <?php foreach ($products as $p):
            ...
            $name         = htmlspecialchars($p['name'] ?? 'Sans nom', ENT_QUOTES, 'UTF-8');
  ...
  <?php else: ?>
    <section>
      ...
      <p>Veuillez retourner à la page d'accueil et choisir une catégorie valide.</p>
    </section>
  <?php endif; ?>
</main>
