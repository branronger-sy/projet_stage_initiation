...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['action']) && $_POST['action'] === 'add_favorite')) {
    ...
    if (!$originOk) {
        json_response(['success' => false, 'message' => 'Origin not allowed'], 403);
    }
    $csrfToken = $_POST['csrf_token'] ?? $_SERVER['HTTP_X_CSRF_TOKEN'] ?? null;
    if (!validateCsrfToken($csrfToken)) {
        json_response(['success' => false, 'message' => 'Invalid CSRF token'], 400);
    }
    ...
    try {
        ...
        if ($exists) {
            json_response(['success' => false, 'message' => 'Already in favorites'], 409);
        }

        $stmt = $pdo->prepare("INSERT INTO favoris (user_id, product_id) VALUES (?, ?)");
        $stmt->execute([$user_id, $product_id]);

        json_response(['success' => true, 'message' => 'Added to favorites'], 201);
    } catch (PDOException $e) {
...
?>
    ...
    <section class="carousel">
        <div class="container">
            <img src="<?= sanitize_image_src('images/carouselhome/Openng sales discounts.png') ?>" alt="Opening Sales Discounts - Argan Oil Products">
            <img src="<?= sanitize_image_src('images/carouselhome/Openng sales discounts (1).png') ?>" alt="Special Offers on Argan Oil - CoopArjana">
        </div>
        <div class="buttons">
                ...
                <?php foreach ($categories as $cat): ?>
                    <div onclick="goToCategory(<?= (int)$cat['id'] ?>)">
                        <img src="<?= sanitize_image_src($cat['main_image'] ?? 'images/default.jpg') ?>" 
                             alt="<?= htmlspecialchars($cat['name_en'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                        <h3><?= htmlspecialchars($cat['name_en'] ?? '', ENT_QUOTES, 'UTF-8') ?></h3>
                    </div>
                <?php endforeach; ?>
            </div>
        </section>
                ...
                <div id="prod-cont">
                    ...
                    if (!empty($popular_products) && is_array($popular_products)) {
                        foreach ($popular_products as $row) {
                            $name = htmlspecialchars($row['name_en'] ?? '', ENT_QUOTES, 'UTF-8');
                        ...
                        foreach ($last_products as $row) {
                            ...
                            <a href="index.php?page=product&id=<?= $product_id ?>" class="product-card-link" rel="noopener noreferrer">
                                <div class="product-card" data-id="<?= $product_id ?>">
                                    ...
                                    <?php else: ?>
                                        <button onclick="event.stopPropagation(); event.preventDefault(); ajouterDepuisProduits(this)">+Add</button>
                                    <?php endif; ?>
                                </div>
                            </a>
                            <?php
